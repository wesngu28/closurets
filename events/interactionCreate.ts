...
export const interactionCreate = async (client: Closure, interaction: Interaction) => {
  ...
  const commandName = Object.keys(command)[0];
	if (!command) return;
  if (commandCooldown.has(`${interaction.guild}.${interaction.commandName}.${interaction.user.id}`)) {
	...
	try {
		...
		setTimeout(() => {
			commandCooldown.delete(`${interaction.guild}.${interaction.commandName}.${interaction.user.id}`);
		}, 10000);
	} catch (error) {
		commandCooldown.delete(`${interaction.guild}.${interaction.commandName}.${interaction.user.id}`);
